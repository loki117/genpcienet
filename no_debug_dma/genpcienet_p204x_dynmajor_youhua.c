...
unsigned char *dma0_channel0_dma_buf = NULL;
...
unsigned char *dma0_channel1_dma_buf = NULL;
unsigned int control0_channel1_buf[MAX_LENGTH];

dma_addr_t control1_dma_addr;
unsigned char *dma1_channel0_dma_buf = NULL;
unsigned int control1_buf[MAX_LENGTH * 2];
unsigned int control1_is_running = 0;
unsigned int control1_is_error = 0;

int dma0_channel0_soft_irq = NO_IRQ;
int dma0_channel1_soft_irq = NO_IRQ;
int dma1_channel0_soft_irq = NO_IRQ;
...
struct fsldma_regs *dma0_mr0_regs;
struct pic_regs *dma0_channel0_pic;

struct fsldma_regs *control0_channel1_regs;
struct pic_regs *dma0_mr1_pic;

struct fsldma_regs *dma1_mr0_regs;
struct pic_regs *dma1_channel0_pic;
...
wait_queue_head_t dmal0_channel1_queue;
struct mutex dma0_channel1_read_lock;
...
//	mutex_lock(&dma_write_lock);
	...
	if(channel == 0)
	{
		...
		if((0x80 == (dma0_mr0_regs->sr & 0x80)))//Transfer error
		{
			control0_channel0_transfer_error++;
			dma0_mr0_regs->sr = 0x80;
			control0_is_running = 0;
		}

		if((0x10 == (dma0_mr0_regs->sr & 0x10)))//Programming error
		{
			control0_channel0_programming_error++;
			dma0_mr0_regs->sr = 0x10;
			control0_is_running = 0;
		}
	 
	 	if(0x4 == (dma0_mr0_regs->sr & 0x4))//Channel busy
	 	{
			if( (control0_channel0_check_length == dma0_mr0_regs->bcr) && (control0_channel0_check_sraddress == dma0_mr0_regs->sar))
			{
				...
				if(control0_channel0_error_check_loop >= 5) //add check time,avoid the last interrupt not run interrupt function
				{
					...
					dma0_mr0_regs->mr = dma0_mr0_regs->mr | (1 << 3);
					control0_is_running = 0;

					for(channel0_loop = 0;channel0_loop <100;channel0_loop++)
					{
						udelay(10);
						if(0x4 != (dma0_mr0_regs->sr & 0x4))
						{
							break;
						}
					}
			...
			{
				control0_channel0_check_length = dma0_mr0_regs->bcr;
				control0_channel0_check_sraddress = dma0_mr0_regs->sar;
				control0_channel0_error_check_loop = 0;
			}
		}
		...
		{
			#if 0
			dma0_mr0_regs->mr = 0x8;
			dma0_mr0_regs->sr = 0xbf;
			#endif
			control0_is_error = 1;
		...
		dma0_mr0_regs->mr = (0x7<< 24) | (0x1<<9) | 0x04;//BWC=0111(Bandwidth=128byte) End-of-segments interrupt enable=1  Channel transfer mode=1 Channel continue=1
		dma0_mr0_regs->satr = (0x5 << 16) |(sa_high);//SREADTTYPE=5(Read, snoop local processor)
		dma0_mr0_regs->sar = sa_low;
		dma0_mr0_regs->datr = (0x5 << 16)  | (control0_dma_addr >> 32);//DWRITETTYPE=5(Write, snoop local processor)
		dma0_mr0_regs->dar = control0_dma_addr;
		dma0_mr0_regs->bcr = sa_len * 4;	
		dma0_mr0_regs->mr = (0x7<< 24) | (0x1<<9) | 0x05;//Channel start
		control0_is_running = 1;		
	}
	else if(channel ==1)
	{
	
		if((0x80 == (dma1_mr0_regs->sr & 0x80) ))
		{
			control1_channel0_transfer_error++;
			dma1_mr0_regs->sr = 0x80;
			control1_is_running = 0;
		}

		if((0x10 == (dma1_mr0_regs->sr & 0x10)))
		{
			control1_channel0_programming_error++;
			dma1_mr0_regs->sr = 0x10;
			control1_is_running = 0;
		}
		/*check dma channel is busy,if 5 times is busy and length ,source address is equal.then think dma error is not working.abort channel*/
	 	if(0x4 == (dma1_mr0_regs->sr & 0x4))
	 	{
			if( (control1_channel0_check_length == dma1_mr0_regs->bcr) && (control1_channel0_check_sraddress == dma1_mr0_regs->sar))
			{
				...
				if(control1_channel0_error_check_loop >= 5)    //add check time to 100
				{
					...
					dma1_mr0_regs->mr = dma1_mr0_regs->mr | (1 << 3);
					control1_is_running = 0;
			...
			{
				control1_channel0_check_length = dma1_mr0_regs->bcr;
				control1_channel0_check_sraddress = dma1_mr0_regs->sar;
				control1_channel0_error_check_loop = 0;
			}
		}
		...
		{
			#if 0
			dma1_mr0_regs->mr = 0x8;
			dma1_mr0_regs->sr = 0xbf;
			#endif
			control1_is_error = 1;
		...
		print("dma1_mr0_regs.mr = 0x%x\n", dma1_mr0_regs->mr);
		print("dma1_mr0_regs.sr = 0x%x\n", dma1_mr0_regs->sr);
		print("dma1_mr0_regs.eclndar = 0x%x\n", dma1_mr0_regs->eclndar);
		print("dma1_mr0_regs.clndar = 0x%x\n", dma1_mr0_regs->clndar);
		print("dma1_mr0_regs.satr= 0x%x\n", dma1_mr0_regs->satr);
		print("dma1_mr0_regs.sar = 0x%x\n", dma1_mr0_regs->sar);
		print("dma1_mr0_regs.datr = 0x%x\n", dma1_mr0_regs->datr);	
		print("dma1_mr0_regs.dar = 0x%x\n", dma1_mr0_regs->dar);	
		print("dma1_mr0_regs.bcr = 0x%x\n", dma1_mr0_regs->bcr);
		control1_loop = 0;
		control1_loop_len =0;
		...
		dma1_mr0_regs->mr = (0x7<< 24) | (0x1<<9) | 0x04;
		dma1_mr0_regs->satr = (0x5 << 16) |(sa_high);
		dma1_mr0_regs->sar = sa_low;
		dma1_mr0_regs->datr = (0x5 << 16)  | (control1_dma_addr >> 32);
		dma1_mr0_regs->dar = control1_dma_addr;
		dma1_mr0_regs->bcr = sa_len*4;	
		dma1_mr0_regs->mr = (0x7<< 24) | (0x1<<9) | 0x05;
...
{	
	...
	switch(cmd)
	{
		...
		case READ_CONTROL0_CHANNEL1:
			mutex_lock(&dma0_channel1_read_lock);
			if (copy_from_user(control0_channel1_arg,(void *)arg,sizeof(unsigned int)*3))
			{
				mutex_unlock(&dma0_channel1_read_lock);
				printk("copy_from_user error\n");
				return -1;
			}
			...
			timeout = 1 * HZ;
			transfer_ok = 0;
			wait_event_timeout(dmal0_channel1_queue,transfer_ok==1,timeout);
			if(transfer_ok == 0)
			{
			...
			memcpy(control0_channel1_buf,dma0_channel1_dma_buf,count * 4);
...
static irqreturn_t dma_control0_channel0_interrupt(int irq,void *dev_instance)
{
	...
	memset(dma0_mr0_regs, 0, sizeof(struct fsldma_regs));
	dma0_mr0_regs->sr = 0x9b;//清各种状态位
	if(control0_buffer_select == 0)
	{
		memcpy(&control0_buf[control0_loop_len], dma0_channel0_dma_buf, control0_arg.addr_size[control0_loop][1]*4);
	...
	if(control0_buffer_select == 1)
	{
		print("control0_loop_len = %d\n",control0_loop_len);
		print("control0_loop = %d\n",control0_loop);
		memcpy(&control0_buf[MAX_LENGTH + control0_loop_len], dma0_channel0_dma_buf, control0_arg.addr_size[control0_loop][1]*4);
		#if 0
		for(i=0;i<6;i++)
		{
	...
	if(control0_loop < control0_arg.num)
	{
		...
		dma0_mr0_regs->mr = (0x7<< 24) | (0x1<<9) | 0x04;
		dma0_mr0_regs->satr = (0x5 << 16) |(sa_high);
		dma0_mr0_regs->sar = sa_low;
		dma0_mr0_regs->datr = (0x5 << 16)  | (control0_dma_addr >> 32);
		dma0_mr0_regs->dar = control0_dma_addr;
		dma0_mr0_regs->bcr = sa_len * 4;	
		dma0_mr0_regs->mr = (0x7<< 24) | (0x1<<9) | 0x05;
		return IRQ_HANDLED;
	}
	control0_is_running = 0;
	if(control0_buffer_select == 0)
...
static irqreturn_t dma_control1_channel0_interrupt(int irq,void *dev_instance)
{
	...
	print("ferry,@ %s\n",__FUNCTION__);
	memset(dma1_mr0_regs,0,sizeof(struct fsldma_regs));
	dma1_mr0_regs->sr = 0x9b;
	if(control1_buffer_select == 0)
	{
		memcpy(&control1_buf[control1_loop_len],dma1_channel0_dma_buf,control1_arg.addr_size[control1_loop][1]*4);
	...
	{
		memcpy(&control1_buf[MAX_LENGTH + control1_loop_len],dma1_channel0_dma_buf,control1_arg.addr_size[control1_loop][1]*4);
		print("control1_loop_len = %d\n",control1_loop_len);
		print("control1_loop = %d\n",control1_loop);
		#if 0
		for(i=0;i<8;i++)
...
//地址信息
#define CCSRBASE_ADDR	0xffe000000//p2020:0xfffe00000 p204x:0xffe000000

#define DMA0_CHANNEL0_IRQ_ADDR	0x50380//p2020(IIVPR4):0x50280  p204x(IIVPR12):0x50380/(IIVPR28)0x50580
#define DMA0_CHANNEL1_IRQ_ADDR	0x503a0//p2020(IIVPR5):0x502a0  p204x(IIVPR13):0x503a0/(IIVPR29)0x505a0
#define DMA1_CHANNEL0_IRQ_ADDR	0x50400//p2020(IIVPR60):0x50980 p204x(IIVPR16):0x50400/(IIVPR32)0x50600

#define DMA0_MR0_ADDR	0x100100//p2020:0x21100 p204x:0x100100
#define DMA0_MR1_ADDR	0x100180//p2020:0x21180 p204x:0x100180
#define DMA1_MR0_ADDR	0x101100//p2020:0xc100  p204x:0x101100

//DMA channel的中断信息
#define dma0_channel0_irq	28//p2020(4+16=20)    p204x(12+16=28)
#define dma0_channel1_irq	29//p2020(5+16=21)    p204x(13+16=29)
#define dma1_channel0_irq	32//p2020(60+16=76)   p204x(16+16=32)

static unsigned int busnum = 0x07;//这里注意要和硬件的实际情况一致
...
static int __init pciedev_probe(struct pci_dev *pci_dev, const struct pci_device_id *pci_id)
{
	...
	mutex_init(&dma0_channel1_read_lock);
	mutex_init(&dma_write_lock);
	mutex_init(&dma_read_lock);
	
	init_waitqueue_head(&dmal0_channel1_queue);
	
	//p2020:DMA0_channel0_irq=4(20) p204x:DMA0_channel0_irq=12(28)
	dma0_channel0_pic = (struct pic_regs *)ioremap(CCSRBASE_ADDR + DMA0_CHANNEL0_IRQ_ADDR, 8);
	if(dma0_channel0_pic == NULL)
	{		 
		return -1;
	}
	dma0_channel0_pic->iivpr |= 0x800000;
	
	//DMA1_MR0 地址
	dma0_mr0_regs=(struct fsldma_regs *)ioremap(CCSRBASE_ADDR + DMA0_MR0_ADDR, 0x24);
	if(dma0_mr0_regs == NULL)
	{
		goto iounmap_dma0_channel0_pic;
	}

	//p2020:DMA0_channel1_irq=5(21) p204x:DMA0_channel1_irq=13(29)
	dma0_mr1_pic =(struct pic_regs *)ioremap(CCSRBASE_ADDR + DMA0_CHANNEL1_IRQ_ADDR, 8);
	if(dma0_mr1_pic == NULL)
	{
		goto iounmap_dma0_mr0_regs;
	}
	dma0_mr1_pic->iivpr |= 0x800000;
	
	//DMA1_MR1 地址
	control0_channel1_regs=(struct fsldma_regs *)ioremap(CCSRBASE_ADDR + DMA0_MR1_ADDR, 0x24);
	if(control0_channel1_regs == NULL)
	{
		goto iounmap_dma0_mr1_pic;
	}

	//p2020:DMA1_channel0_irq=60(76) p204x:DMA1_channel0_irq=16(32)
	dma1_channel0_pic =(struct pic_regs *)ioremap(CCSRBASE_ADDR + DMA1_CHANNEL0_IRQ_ADDR, 8);
	if(dma1_channel0_pic == NULL)
	{		
		goto iounmap_control0_channel1_regs;
	}
	dma1_channel0_pic->iivpr |= 0x800000;
	
	//DMA2_MR0
	dma1_mr0_regs=(struct fsldma_regs *)ioremap(CCSRBASE_ADDR + DMA1_MR0_ADDR, 0x24);
	if(dma1_mr0_regs == NULL)
	{		
		goto iounmap_dma1_channel0_pic;
	}
	
	//DMA0_channel0_irq中断申请，并挂处理函数
	dma0_channel0_soft_irq = irq_create_mapping(NULL, dma0_channel0_irq);
	if(dma0_channel0_soft_irq == NO_IRQ)
	{
		printk("irq mapping failed!\n");
		goto iounmap_dma1_mr0_regs;
	}		
	ret = request_irq(dma0_channel0_soft_irq, dma_control0_channel0_interrupt, 0, "dma0-channel0", NULL);
	if(ret != 0)
	{
	...
	//DMA0_channel1_irq中断申请，并挂处理函数
	dma0_channel1_soft_irq = irq_create_mapping(NULL, dma0_channel1_irq);
	if(dma0_channel1_soft_irq == NO_IRQ)
	{
		printk("irq mapping failed!\n");
		goto free_control0_irq;
	}		
	ret = request_irq( dma0_channel1_soft_irq, dma_control0_channel1_interrupt, 0, "dma0-channel1", NULL);
	if(ret != 0)
	{
		printk("request irq failed!\n");
		goto free_control0_irq;
	...
	//DMA1_channel0_irq中断申请，并挂处理函数
	dma1_channel0_soft_irq = irq_create_mapping(NULL, dma1_channel0_irq);
	if(dma1_channel0_soft_irq == NO_IRQ)
	{	
		printk("irq mapping failed!\n");
		goto free_control0_channel1_irq;
	}		
	ret = request_irq( dma1_channel0_soft_irq, dma_control1_channel0_interrupt, 0, "dma1-channel0", NULL);
	if(ret != 0)
	{
		printk("request irq failed!\n");
		goto free_control0_channel1_irq;
	...
	//PCI设备申请DMA一致性缓冲区
	dma0_channel0_dma_buf = pci_alloc_consistent(pci_dev, MAX_LENGTH * 4, &control0_dma_addr);
	if(!dma0_channel0_dma_buf)
	{
		printk("could not allocate coherent DMA buffer.\n");
		goto free_control0_channel1_irq;
	}

	dma0_channel1_dma_buf = pci_alloc_consistent(pci_dev,MAX_LENGTH * 4, &control0_channel1_dma_addr);
	if(!dma0_channel1_dma_buf)
	{
		printk("could not allocate coherent DMA buffer.\n");
		goto free_dma0_channel0_dma_buf;		
	}

	dma1_channel0_dma_buf = pci_alloc_consistent(pci_dev,MAX_LENGTH * 4,&control1_dma_addr);
	if(!dma1_channel0_dma_buf)
	{
		printk("could not allocate coherent DMA buffer.\n");
		goto free_dma0_channel1_dma_buf;
	}
	
	print("dma0_channel0_dma_buf = 0x%x\n",dma0_channel0_dma_buf);
	print("dma0_channel1_dma_buf = 0x%x\n",dma0_channel1_dma_buf);
	print("dma1_channel0_dma_buf = 0x%x\n",dma1_channel0_dma_buf);
	...
	//这里需要和预设的全部都对上才能往下走，注意p204x平台上具体值可能有不同
...
free_dma0_channel1_dma_buf:
	pci_free_consistent(pci_dev,MAX_LENGTH * 4,dma0_channel1_dma_buf,control0_channel1_dma_addr);
free_dma0_channel0_dma_buf:
	pci_free_consistent(pci_dev,MAX_LENGTH * 4,dma0_channel0_dma_buf,control0_dma_addr);
free_control0_channel1_irq:
	free_irq(dma0_channel1_soft_irq, NULL);
free_control0_irq:
	free_irq(dma0_channel0_soft_irq,NULL);
iounmap_dma1_mr0_regs:
	iounmap(dma1_mr0_regs);
iounmap_dma1_channel0_pic:
	iounmap(dma1_channel0_pic);
iounmap_control0_channel1_regs:
	iounmap(control0_channel1_regs);
iounmap_dma0_mr1_pic:
	iounmap(dma0_mr1_pic);
iounmap_dma0_mr0_regs:
	iounmap(dma0_mr0_regs);
iounmap_dma0_channel0_pic:
	iounmap(dma0_channel0_pic);
...
static int pciedev_major = 0;  /* default to dynamic major */
static int pciedev_minor = 0;
#define PCIE_MODULE_NAME 	"pciedev"
#define PCIE_CLASS_NAMW		"pcie_dev_class"
...
static int __init pciedev_init_module(void)
{
	...
	{
		ret = alloc_chrdev_region(&devno, 0, 1, PCIE_MODULE_NAME);
		pciedev_major = MAJOR(devno);
	}
	if (ret < 0) 
	{
	...
	pcie_dev_class = class_create(THIS_MODULE, PCIE_CLASS_NAMW);
...
static void __exit pciedev_cleanup_module(void)
{
	...
	while (!list_empty(&device_list)) 
	{
			...
			(tmp_pcidev->funcnum == funcnum))
		{
			...
			pci_free_consistent(tmp_pcidev->pcidev, MAX_LENGTH * 4, dma1_channel0_dma_buf, control1_dma_addr);
			pci_free_consistent(tmp_pcidev->pcidev, MAX_LENGTH * 4, dma0_channel1_dma_buf, control0_channel1_dma_addr);
			pci_free_consistent(tmp_pcidev->pcidev,MAX_LENGTH * 4, dma0_channel0_dma_buf, control0_dma_addr);
			free_irq(dma1_channel0_soft_irq, NULL);
			free_irq(dma0_channel1_soft_irq, NULL);
			free_irq(dma0_channel0_soft_irq, NULL);
			iounmap(dma1_mr0_regs);
			iounmap(dma1_channel0_pic);
